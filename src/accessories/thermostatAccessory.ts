...
export class ThermostatAccessory {
  ...
  private getCurrentTempFromContext(): number {
    ...
    if (sensorAccessory) {
      const sensorDevice: ParsedDevice = sensorAccessory.context.device;
  ...
  async getCurrentHeatingCoolingState(): Promise<CharacteristicValue> {
    ...
    if (heaterAccessory) {
      const heaterDevice: ParsedDevice = heaterAccessory.context.device;
